...
alive4 = evo4.get_alive()
flagged4 = [a for a in alive4 if a.flagged_sybil]
...
flagged_final = [a for a in evo_full.get_alive() if a.flagged_sybil]
...
# Verify flagged agents are still alive (quarantine, not death)
flagged_alive = [evo_repro.agents[sid] for sid in sybil_ids
                 if sid in evo_repro.agents and evo_repro.agents[sid].alive]
test("Flagged agents survive in quarantine",
     len(flagged_alive) > 0 or not any(sid in evo_repro.agents for sid in sybil_ids),
     f"flagged_alive={len(flagged_alive)}")
